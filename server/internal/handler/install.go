...
func (h *InstallHandler) Manifest(c *gin.Context) {
	...
	bundleIdentifier := "com.susuq.JiSign"
	appTitle := "速签"
...
// SignWithCert 接收用户上传的 P12 + mobileprovision，服务端用 zsign 签名速签 App 后返回安装链接
// POST /api/install/sign-with-cert
func (h *InstallHandler) SignWithCert(c *gin.Context) {
	// 获取上传的文件
	p12File, err := c.FormFile("p12")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "error": "缺少 P12 证书文件"})
		return
	}

	mpFile, err := c.FormFile("mobileprovision")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "error": "缺少描述文件"})
		return
	}
	...
	// 检查未签名 IPA 是否存在
	if _, err := os.Stat(h.UnsignedIPA); os.IsNotExist(err) {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": "服务端签名文件缺失，请联系管理员"})
		return
	}

	// 创建临时目录保存上传的证书文件
	tmpDir, err := os.MkdirTemp(h.IPADir, "sign-cert-")
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": "创建临时目录失败"})
		return
	}
	...
	if err := c.SaveUploadedFile(p12File, p12Path); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": "保存 P12 文件失败"})
		return
	}
	if err := c.SaveUploadedFile(mpFile, mpPath); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": "保存描述文件失败"})
		return
	}

	// 生成唯一 token
	token := "cert_" + uuid.New().String()
	signedDir := filepath.Join(h.IPADir, "signed")
	os.MkdirAll(signedDir, 0755)
	outputIPA := filepath.Join(signedDir, token+".ipa")
	...
	output, err := cmd.CombinedOutput()
	if err != nil {
		log.Printf("签名失败: %v, output: %s", err, string(output))
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": "签名失败: " + string(output)})
		return
	}

	// 保存元数据
	metaJSON := fmt.Sprintf(`{"bundle_id":"com.jisign.JiSign","app_name":"速签"}`)
	os.WriteFile(filepath.Join(signedDir, token+".json"), []byte(metaJSON), 0644)

	installURL := "itms-services://?action=download-manifest&url=" + h.BaseURL + "/api/install/manifest/" + token
	log.Printf("用户证书签名完成: token=%s", token)
